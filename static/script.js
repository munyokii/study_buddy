class FlashcardApp {
  ...
  async generateFlashcards() {
    ...
    try {
      ...
      if (data.success && data.flashcards) {
        ...
        this.showToast(`Generated ${data.flashcards.length} flashcards successfully!`, 'success');
  ...
  displayFlashcards() {
    ...
    if (currentCardSpan) {
      currentCardSpan.textContent = this.currentCardIndex + 1;
    }
    if (totalCardsSpan) {
      totalCardsSpan.textContent = this.flashcards.length;
    }
  ...
  saveFlashcardsToLocalStorage() {
    try {
      ...
      localStorage.setItem('saved_flashcards', JSON.stringify(flashcardData));
    } catch (error) {
  ...
  async loadSavedFlashcards() {
    try {
      await this.loadFlashcardsFromServer();
    } catch (error) {
  ...
  loadFlashcardsFromLocalStorage() {
    try {
      const savedData = localStorage.getItem('saved_flashcards');
      if (savedData) {
        ...
        if (data.flashcards && data.flashcards.length > 0) {
          ...
          this.showToast('No saved flashcards found.', 'info');
        }
      } else {
        this.showToast('No saved flashcards found.', 'info');
      }
      ...
      this.showToast('Error loading saved flashcards.', 'error');
    }
  }
  ...
  clearFlashcards() {
    ...
    this.showToast('All flashcards cleared.', 'success');
  }

  exportFlashcards() {
    if (this.flashcards.length === 0) {
      this.showToast('No flashcards to export.', 'error');
      return;
    }
      ...
      try {
        ...
        if (Array.isArray(importedCards) && importedCards.length > 0) {
          ...
          if (isValid) {
            ...
            this.showToast(`Imported ${importedCards.length} flashcards successfully!`, 'success');
          } else {
        ...
        this.showToast('Error importing flashcards. Please check the file format.', 'error');
      }
    };
